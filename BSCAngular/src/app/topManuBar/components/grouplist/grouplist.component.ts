...
ngOnInit() {
  ...
  this.rowData.subscribe(
      ...
      data => { this.resData = data;
                ...
                // debugger;
                if(counterCheck === 0 ){
                    this.columnDefs.push({ field: 'col' + counter, headerName: element,
                ...
                this.columnDefs.push({ field: 'groupKey', sortable: true,  filter: true,
                 });
                this.columnDefs.push({ field: 'groupName' , sortable: true, filter: true });
            },
      (error) => alert(error),
...
public onEditBtnClick() {
    ...
    localStorage.setItem('LVselectedRowId', this.grid.api.getSelectedRows()[0].col1);
